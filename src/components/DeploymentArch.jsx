...
const TOOLS = {
  ...
  juniper:      { label:"Juniper Edge Routers", icon:"🌐", zone:"f35",      method:"collector",   color:"#FF8C00", controls:["SC-7","CM-6"],          detail:"SSH to pull running-config. RiskRadar compares vs Network Infrastructure STIG. Read-only SSH key, no enable/write access." },
...
const STEPS = [
  {
    ...
    desc:"For tools that are already SaaS (CrowdStrike, Zscaler), RiskRadar connects directly via their APIs. The ISSO submits a request to LM for a read-only API key. LM creates the key, ISSO enters it in RiskRadar. No network changes, no agents, no on-site work.",
    actions:[
      "ISSO submits ticket to LM: 'Request read-only CrowdStrike Falcon API key for RiskRadar integration'",
      "LM creates API client_id + client_secret in Falcon console with read-only scope",
      "ISSO enters credentials in RiskRadar Connector Settings (stored in AWS Secrets Manager)",
      "RiskRadar polls CrowdStrike every 5 minutes — Spotlight CVEs + Detections flow in automatically",
    ],
    effort:"1–2 days",
  },
  {
    ...
    desc:"Tenable.sc lives inside LM's network infrastructure — it's on-prem, not cloud. RiskRadar can't reach it directly over the internet. Two options: file export (ISSO or LM admin exports .nessus → ISSO uploads) or an API bridge if a network path can be established through LM.",
    actions:[
      "RECOMMENDED — File Import: LM Tenable.sc admin exports scan as .nessus → sends to ISSO via secure means → ISSO drags file into RiskRadar",
  ...
  },
  {
    ...
    desc:"Systems inside the F-35 program boundary — AD, Windows machines, Juniper/Cisco network devices — cannot be reached from GovCloud directly. A lightweight Collector Agent runs inside the boundary, makes read-only queries to these systems, and pushes encrypted results to RiskRadar over outbound HTTPS. The ISSO or program admin deploys this via a change request.",
    actions:[
      "Submit Change Request: Deploy RiskRadar Collector Agent on a dedicated VM inside the F-35 boundary",
      "Admin runs ONE command: docker run -e TOKEN=xxx -e ORG=xxx sentinelgrc/collector:latest",
      "Submit Firewall Change Request: Allow collector VM → RiskRadar GovCloud IPs on 443/TCP outbound ONLY",
  ...
  },
  {
    ...
    color:"#AA66FF",
    tools:["awssechub"],
    desc:"If the program has an AWS GovCloud presence, Security Hub is already in the same region as RiskRadar. IAM role-based access — no credentials to manage, no firewall rules, no agents.",
    actions:[
      "Create IAM role in customer's GovCloud account with SecurityHub:GetFindings permission",
      "RiskRadar assumes the role via cross-account trust relationship",
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"'Helvetica Neue',Arial,sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:C.headerBg, borderBottom:`1px solid ${C.border}`, padding:"10px 20px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:C.white }}>RiskRadar — Deployment Architecture</div>
            ...
            <div style={{ display:"flex", gap:12, alignItems:"stretch", minHeight:480 }}>
              ...
              <Zone title="SENTINELGRC" badge="AWS GOVCLOUD IL4" color={C.teal} width="22%"
                ...
                <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:7, padding:10 }}>
                  ...
                  <div style={{ fontSize:10, color:C.dim }}>Browser → CAC auth → JWT → RiskRadar dashboard</div>
              ...
              <Zone title="F-35 PROGRAM BOUNDARY" badge="NIPR / IL4 BOUNDARY" color={C.blue} width="32%"
                ...
                <div style={{ background:"rgba(255,140,0,0.06)", border:"1px solid rgba(255,140,0,0.2)", borderRadius:6, padding:8 }}>
                  ...
                  <div style={{ ...mono, fontSize:10, color:C.dim, marginTop:3, lineHeight:1.6 }}>
                    Collector VM → RiskRadar GovCloud IPs<br/>
          ...
          <div style={{ maxWidth:700 }}>
            ...
            {[
              { to:"LM IT Service Desk", color:"#E91E63", items:[
                { task:"CrowdStrike Falcon read-only API key for RiskRadar", detail:"Scope: spotlight:read, detects:read, hosts:read, vulnerabilities:read", urgent:true },
              ...
              { to:"Program Network/Firewall Admin", color:C.orange, items:[
                { task:"Firewall rule: Collector VM → RiskRadar GovCloud", detail:"10.x.x.x → 52.61.0.0/16 (AWS GovCloud us-gov-west-1) → 443/TCP outbound ONLY", urgent:true },
              ...
              { to:"Program System/Server Admin", color:C.blue, items:[
                { task:"Deploy RiskRadar Collector Agent on dedicated VM", detail:"docker run sentinelgrc/collector OR Windows Service install — 30 minutes", urgent:true },
